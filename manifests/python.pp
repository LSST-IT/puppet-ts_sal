class ts_sal::python{
	...
	exec{ "get-custom-python":
		...
		onlyif => "test ! -f /tmp/python3-to-install.tgz  && ! -f /usr/local/bin/python3.6"
	}
	exec{ 'unpack-custom-python':
		...
		onlyif => "test ! -f /tmp/python3-to-install.tar  && ! -f /usr/local/bin/python3.6"
	}
	...
	exec{ 'install-custom-python':
		...
		onlyif => "test ! -d /tmp/Python-3.6.3/  && ! -f /usr/local/bin/python3.6"
	}
	...
	exec{ 'install-numpy':
		...
		onlyif => "test -z \"$(pip3 list --format=columns 2>/dev/null | grep numpy)\""
	}
